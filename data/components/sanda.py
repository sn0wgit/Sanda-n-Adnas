...
class Sanda(pg.sprite.Sprite):
    ...
    def load_images_from_sheet(self):
        ...
        self.right_small_normal_frames.append(
            self.get_image(226, 1, 24, 31))   # Right [0]
        self.right_small_normal_frames.append(
            self.get_image(258, 1, 24, 31))   # Right walking 1 [1]
        self.right_small_normal_frames.append(
            self.get_image(290, 1, 24, 31))   # Right walking 2 [2]
        self.right_small_normal_frames.append(
            self.get_image(354, 1, 24, 31))   # Right walking 3 [3]
        self.right_small_normal_frames.append(
            self.get_image(321, 1, 24, 31))   # Right jump [4]
        self.right_small_normal_frames.append(
            self.get_image(228, 1, 22, 31))   # Right skid [5]
        self.right_small_normal_frames.append(
            self.get_image(0, 1, 22, 31))     # Death frame [6]
        self.right_small_normal_frames.append(
            self.get_image(195, 1, 24, 31))  # Flag pole slide [7]
    ...
    def standing(self, keys, fire_group):
        ...
        if keys[tools.keybinding['left_a']]:
            self.facing_right = False
            self.get_out_of_crouch()
            self.state = c.WALK
        elif keys[tools.keybinding['right_a']]:
            self.facing_right = True
            self.get_out_of_crouch()
            self.state = c.WALK
        elif keys[tools.keybinding['jump_a']]:
            if self.allow_jump:
        ...
        if not keys[tools.keybinding['down_a']]:
            self.get_out_of_crouch()
    ...
    def check_to_allow_jump(self, keys):
        ...
        if not keys[tools.keybinding['jump_a']]:
            self.allow_jump = True
    ...
    def walking(self, keys, fire_group):
        ...
        if keys[tools.keybinding['jump_a']]:
            if self.allow_jump:
                if self.big: setup.SFX['big_jump'].play()
                else: setup.SFX['small_jump'].play()
                self.state = c.JUMP
    ...
    def jumping_to_death(self):
        """Called when Sanda is in a DEATH_JUMP state"""
    ...
    def start_death_jump(self, game_info):
        """Used to put Sanda in a DEATH_JUMP state"""
        self.dead = True
        game_info[c.SANDA_DEAD] = True
        self.y_vel = -11
        self.gravity = .5
        self.frame_index = 6
        self.image = self.right_frames[self.frame_index]
